...
const registerUser = asyncHandler( async (req, res) => {
    // get user details from f.end
    // Validation - not empty
    // check if user already exists: thru username or email
...
const loginUser = asyncHandler( async (req, res) => {
    // re body -> data
    // username or email {login criteria}
    // find the user
    // password check
    ...
    const user = await User.findOne({
      ...
      const { accessToken, refreshToken} =  await generateAccessAndRefreshToken(user._id)
       ...
       .json( // when user wants to save accessToken, refreshToken by its own :
        new ApiResponse(
            200, {
    ...
    if (!isPasswordCorrect) {
      ...
      const updateAccountDetails = asyncHandler(async (req, res) => {
       ...
       const user = User.findByIdAndUpdate(
        ...
        return res
        .status(200)
        .json(new ApiResponse(200, user, "account status updated"))
